...
其中 $v(x,t)=f(x,t)-\frac{1}{2}g^2(t)\nabla_x\log p_t(x)$。进而：(i) PF-ODE $\frac{d\tilde{x}(t)}{dt}=v(\tilde{x}(t),t)$ 从 $\tilde{x}(0)\sim p_0$ 正向或 $\tilde{x}(T)\sim p_T$ 反向积分，均有 $\tilde{x}(t)\sim p_t$；(ii) 反向时间 SDE 式 (4.1.6) 从 $\bar{x}(0)\sim p_T$ 出发，有边际 $\bar{x}(t)\sim p_{T-t}$。
